...
actor {
  ...
  // Chain Image Management
  let chainImages = Map.empty<Text, Storage.ExternalBlob>();
  ...
  // Chain Image Methods
  public query ({ caller }) func listChainImages() : async [(Text, Storage.ExternalBlob)] {
    chainImages.toArray();
  };

  public query ({ caller }) func getChainImage(chainKey : Text) : async ?Storage.ExternalBlob {
    chainImages.get(chainKey);
  };

  public shared ({ caller }) func setChainImage(chainKey : Text, imageId : Storage.ExternalBlob) : async () {
    if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
      Runtime.trap("Unauthorized: Only admins can perform this action");
    };
    chainImages.add(chainKey, imageId);
  };

  public shared ({ caller }) func deleteChainImage(chainKey : Text) : async () {
    if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
      Runtime.trap("Unauthorized: Only admins can perform this action");
    };
    chainImages.remove(chainKey);
  };
  ...
  // Bookings list is protected by frontend password -- open query so admin panel works without II
  public query ({ caller }) func listBookings() : async [Booking] {
    bookings.values().toArray().sort();
  };

  // Delete booking -- open to any caller; frontend password gates access
